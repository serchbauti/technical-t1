...
def test_charge_approval_flow(test_client) -> None:
    client = create_client(test_client)
    card = create_card(test_client, client_id=client["id"], pan=create_pan())

    charge = create_charge(test_client, client_id=client["id"], card_id=card["id"], amount=200.0)
    assert charge["status"] == "approved"
    assert charge["reason_code"] is None

    refund_resp = test_client.post(f"/charges/{charge['id']}/refund")
    ...
    second_refund = test_client.post(f"/charges/{charge['id']}/refund")
    assert second_refund.status_code == 409


def test_charge_declined_by_rules(test_client) -> None:
    client = create_client(test_client)

    blocked_card = create_card(
        test_client,
        client_id=client["id"],
        pan=generate_pan_with_last4("0000"),
    )
    blocked_charge = create_charge(
        test_client,
        client_id=client["id"],
    ...
    high_amount_card = create_card(test_client, client_id=client["id"], pan=create_pan())
    high_amount_charge = create_charge(
        test_client,
        client_id=client["id"],
...
def test_charge_idempotency(test_client) -> None:
    client = create_client(test_client)
    card = create_card(test_client, client_id=client["id"], pan=create_pan())
...
def test_charge_listing_filters(test_client) -> None:
    client = create_client(test_client)
    card = create_card(test_client, client_id=client["id"], pan=create_pan())

    approved_charge = create_charge(
        test_client,
        client_id=client["id"],
        card_id=card["id"],
        amount=100.0,
    )
    declined_charge = create_charge(
        test_client,
        client_id=client["id"],
        card_id=card["id"],
        amount=6000.0,
    ...
    refund_resp = test_client.post(f"/charges/{approved_charge['id']}/refund")
    assert refund_resp.status_code == 200
    history_resp = test_client.get(f"/charges/{client['id']}")
